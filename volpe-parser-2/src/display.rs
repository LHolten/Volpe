...
//             Ok(())
//         }
//         Contained::Brackets { inner, .. } => inner
//             .as_ref()
//             .map(|inner| display_syntax(f, inner, indent, hide_semicolon))
//             .unwrap_or(Ok(())),
//         Contained::Terminal(Ok(lexeme)) => with_indent2(f, lexeme.kind, lexeme.text, indent),
//         Contained::Terminal(Err(_)) => with_indent(f, None::<()>, indent),
//     }
// }

// impl<E> fmt::Display for Contained<'_, E> {
